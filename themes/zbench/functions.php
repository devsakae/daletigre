...
class zBenchOptions {
	...
	function init() {
		if(isset($_POST['classic_save'])) {
			...
			$options['nav_id'] = stripslashes($_POST['nav_id']);			
	...
	<div class="wrap">
		...
		<table class="form-table">
			<tbody>
				<tr valign="top">
					<th scope="row">
						<?php _e('<span style="font-weight:bold;">Navigation Bar display</span>', 'classic'); ?>
					</th>
					<td>
						<!-- nav_id -->
						<select style="width:240px;" name="nav_id" id="nav_id">
							<?php if($options['nav_id'] == 'pages'){ ?>
							<option selected="selected">pages</option><option>categories</option>
							<?php } ?>
							<?php if($options['nav_id'] == 'categories'){ ?>
							<option selected="selected">categories</option><option>pages</option>
							<?php } ?>
						</select>
						<br/>
						<small style="font-weight:normal;"><?php _e('You can choose to display a list of CATEGORIES or a list of PAGES in the navigation bar. (Default is PAGES)', 'classic'); ?></small>
					</td>
				</tr>
			</tbody>
		</table>
